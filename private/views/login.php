...
// Controle des données envoyées par l'utilisateur
    ...
    if($save)
    {
    	...
    	{
    		$save=false;
    		setFlashbag("danger", "Erreur lors de l'identification.");
    	}
    }
    else 
    {
    	// Message d'erreur avec flashbag
    }
    ...
    if ($save && $user) 
    {
        ...
        $pwd_hash=$user->password; /*-> est un opérateur d'objet en php.*/
        ...
        "email"=>$user->email,
        "roles"=>explode(ROLE_GLUE, $user->roles)
        ];
