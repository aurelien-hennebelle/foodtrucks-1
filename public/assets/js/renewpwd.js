...
function sendRenewPwdRequest() {
  ...
  // -> doit contenir au plus 16 caractères
    ...
    send = false;
    $('#pwd_new').before('<div class="text-danger">Le mot de passe doit contenir au max 16 caractères.</div>');
  }
  // -> doit avoir au moins un caractère de type numérique
  else if (!/\d/.test(pwd_new)) {
    send = false;
    $('#pwd_new').before('<div class="text-danger">Le mot de passe doit contenir un caractère numérique.</div>');
  }
  ...
  // -> doit avoir au moins un caractère spécial (#@!=+-_)
    ...
    send = false;
    $('#pwd_new').before('<div class="text-danger">Le mot de passe doit contenir un caractère spécial.</div>');
  }
